...
import { updateBucketConnection } from "@/actions/bucket-actions";
...
import type { BucketConnection } from "@/lib/types";
...
export function EditConnectionForm({ connection }: EditConnectionFormProps) {
	...
	async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			const response = await updateBucketConnection(connection.id, data);
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						...
						<div className="space-y-2">
							...
							<Input
								id="bucket"
								name="bucket"
								defaultValue={connection.bucket || ""}
