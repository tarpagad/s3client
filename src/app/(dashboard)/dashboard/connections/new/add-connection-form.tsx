...
import { addBucketConnection } from "@/actions/bucket-actions";
...
export function AddConnectionForm() {
	...
	async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			const response = await addBucketConnection(data);
			...
			if (response.error) {
				...
				toast.success(
					type === "s3"
						? "S3 connection added successfully"
						: "R2 connection added successfully",
				);
				...
				<CardContent className="space-y-6">
					...
					<div className="space-y-2">
						...
						<Input
							...
							name="name"
							placeholder={
								type === "s3" ? "e.g. Production S3" : "e.g. Media R2"
							}
							required
						/>
					</div>
					...
					{type === "r2" && (
						<div className="space-y-2">
							...
							<p className="text-xs text-muted-foreground">
								Your R2 endpoint URL from the Cloudflare dashboard. Required for
								R2 connections.
							</p>
						</div>
					)}
